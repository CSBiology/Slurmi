...
type Job (jobName: string,processList:(string*string list)list)= 
    ...
    member this.formatOneDash (job:Job) (str:System.Text.StringBuilder) = 
        job.OneDash.GetProperties(true)
        |> Seq.toArray
        |> Array.filter (fun x -> x.Key <> "Name")
        |> Array.map (fun x -> str.AppendLine (sprintf "-%s %s" x.Key (x.Value.ToString()))) |> ignore

    member this.formatTwoDashes (job:Job) (str:System.Text.StringBuilder) = 
        job.OneDash.GetProperties(true)
        |> Seq.toArray
        |> Array.filter (fun x -> x.Key <> "Name")
        |> Array.map (fun x -> str.AppendLine (sprintf "--%s=%s" x.Key (x.Value.ToString()))) |> ignore

    member this. formatOnlyKey (job:Job) (str:System.Text.StringBuilder) = 
        job.OnlyKey.GetProperties(true)
        |> Seq.toArray
        |> Array.filter (fun x -> x.Key <> "Name")
        |> Array.map (fun x -> str.AppendLine (sprintf "--%s" x.Key)) |> ignore


    member this.getEnvironmentValue (job: Job) =
        let environmentOption = job |> Job.tryGetEnvironment |> Option.map (fun value -> (value :?> EnvironmentSLURM).GetEnvironment())
        environmentOption |> Option.toList |> List.head

    member this.formatEnv (job:Job) (str:System.Text.StringBuilder) = 
        let env = this.getEnvironmentValue job 
        env 
        |> Seq.toArray 
        |> Array.map (fun x -> str.AppendLine (sprintf "%s %s" (fst x) (snd x))) |> ignore 



    member this.formatProcesses (job:Job) (str:System.Text.StringBuilder) =

        for (processX, args) in job.Processes do
            str.AppendFormat("{0} {1}\n", processX, (String.concat " " args)) |> ignore

    member this.formatAll (job:Job) (str:System.Text.StringBuilder)=
        str.AppendFormat (sprintf "-J %s" job.Name) |> ignore
        this.formatOneDash job str       |> ignore
        this.formatTwoDashes job str     |> ignore 
        this.formatOnlyKey job str       |> ignore 
        this.formatEnv job str           |> ignore
        this.formatProcesses job str     |> ignore
...
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (Job.formatTime (arg.Value:?>(int*int*int*int))))) |> ignore 
...
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (((arg.Value:?>string list))|> String.concat ""))) |> ignore 
...
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (((arg.Value:?>string list))|> String.concat ","))) |> ignore 
//            elif arg.Key = "--time-min=" then 
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (Job.formatTime (arg.Value:?>(int*int*int*int))))) |> ignore 
//            elif arg.Key = "--acctg-freq=" then 
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, ((arg.Value:?>(AccountingFrequency*int)[] |> Array.map (fun (fst,snd) -> (fst.ToString()+ "=" + (snd |> string)) )) |> String.concat "," ))) |> ignore 
//            elif arg.Key = "-a " then 
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (arg.Value:?>ArraySlurm |> ArraySlurm.getArray))) |> ignore 
//            elif arg.Key = "--dependency=" then 
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, (DependencyType.concat (fst (arg.Value:?>(TypeOfDep*DependencyType[]))) (snd (arg.Value:?>(TypeOfDep*DependencyType[])))))) |> ignore 
//            //elif arg.Key = "begin" then 
//            //    str.AppendFormat("#SBATCH {0}{1}\n",  dictForSLURM.Item arg.Key, ((arg.Value:?>string ))) |> ignore 
//            elif arg.Key = "--wait-all-nodes=" then 
//                str.AppendFormat("#SBATCH {0}{1}\n",  (arg.Key, 1)) |> ignore 
//            elif arg.Value = true  then 
//                str.AppendFormat("#SBATCH {0}\n",  ( arg.Key)) |> ignore 
